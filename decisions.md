...
## Goal-section A/B: inconclusive, because `goal-relevance` and `entropy` contradict each other
- **Date**: 2026-09-13
- **Method**: Blind executor A/B over the four `document-style` evals. A = skill at `origin/main`, B = the same skill plus the `## Goal` section. Each arm's `SKILL.md` was injected identically via `claude -p --safe-mode --append-system-prompt`, so neither arm saw the machine's installed skills, CLAUDE.md, or hooks. 2 runs per eval per arm (16 runs); one fresh grader per run, seeing only the produced document and the assertion list — never the arm.
- **Scores**: A 60/68 assertions passed (88%), B 58/68 (85%). Split by eval type: generative (design doc, Japanese explainer) A 32/34, B 34/34; revision (`revise-*`) A 28/34, B 24/34. `goal-relevance`, the assertion added with the section: A 8/8, B 6/8. `information-preserved`: A 4/4, B 4/4.
- **Root cause of the `goal-relevance` result**: clause 1 of the assertion ("names what it should let the reader decide, do, or understand") rewards the sentence that the skill's own no-announcement rule deletes, so it fights `entropy` in the same eval. On `revise-overwrought-rollout` the two assertions traded places run for run: A/2 and B/2 each wrote "This document describes … so that …", each passed `goal-relevance` citing that sentence, and each failed `entropy` citing the same sentence; B/1 deleted the announcement, passed `entropy`, and failed `goal-relevance` for stating "no reader-oriented purpose". Clause 1 is also graded inconsistently — A/1 passed it on title-plus-thesis alone, which B/1 was failed for.
- **Decision**: The `## Goal` section stays, as the user requested. The measurement neither supports nor refutes it: the one assertion that was supposed to test it is measuring an artifact of its own wording.
- **Decision**: Narrow `goal-relevance` to its second clause — every sentence advances what the document is for, with no true-but-irrelevant background — in all four evals. Clause 1 duplicated `overview-first` and collided with `entropy`. The narrowed assertion is unmeasured; the next run of these evals is its first.
- **Over-deletion hypothesis**: raised mid-run when a baseline revision kept a phase table and a concrete 1% → 2% → 4% → 8% example that the treatment run dropped. Not supported and not refuted. `information-preserved` held 4/4 in both arms, so no hard fact was lost; every B loss fell on the revision evals (`complex-fact-example`, `no-back-references`, `inline-references`), but each is a single run.
- **Unrelated findings, present in both arms**: `bold-skim-layer` fails 0/2 in both arms on `revise-overwrought-rollout` because the assertion forbids bold on labels while the skill's Emphasis section explicitly permits it at the head of a list item — the assertion and the skill disagree, and one of them should change. `complex-fact-example` is weak in both arms (A 1/2, B 0/2): executors restate the doubling rule abstractly instead of giving a step sequence.
- **Method caveats**: n=2 per cell, so a 60-vs-58 split is noise. The runner filled its parallel pool in list order, so all arm-A runs executed before all arm-B runs rather than interleaved; grading was blind and the skill difference was injected deterministically, so this does not bias the comparison, but it is not a controlled ordering.

## DR: Drafting Process placement
- **Date**: 2026-09-16
- **Context**: Reviewing `document-style` against Vitter's "Structure + Style = Communication" (2011) found product rules and revision tools but no composition procedure. Version-by-version refinement is the one rule Vitter marks as most important.
- **Decision**: A short section in `document-style`, gated to writing from scratch, with the worked example in `references/drafting-process.md`. Not a separate skill.
- **Rationale**: "write a doc" is one trigger and should load rules and process together. The process exists to produce the structure the Top-Down Structure section demands. Four steps do not justify a skill. Matches the earlier move of the verification loop into `references/`. A separate skill only makes sense for an outline-approval workflow, which can call `document-style` later without moving anything.
- **Scope of the same review**: adopted sentence order (active voice, verb near subject, known before new) and the no-bare-this rule; deferred phrase-level fillers, italics for defined terms, and an English-mechanics reference file; rejected Vitter's ban on singular "they" as outdated.

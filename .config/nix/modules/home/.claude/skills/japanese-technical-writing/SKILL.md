...
description: >
  Japanese technical document writing standards for books, chapters, articles,
  and explanatory texts. Covers formatting, paragraph and argument structure,
  logical precision, reader cognitive load management, perspective and narration,
  restraint in presentation, prohibition of LLM-like empty phrases, and
  elimination of redundancy. Use when writing or revising Japanese technical
  manuscripts — especially when editing drafts for technical books or reviewing
  prose for clarity, logical consistency, and style.
---
...
この規範は、常時適用される `response-style` ルール（`.claude/rules/response-style.md`）の上に、日本語固有の規則を重ねたものである。
結論を先に述べること、一文に一つの意味だけを持たせること、同じ概念には同じ語を使うこと、事実と推測を書き分けることは `response-style` 側の土台であり、日本語の原稿でも同じように適用する。
本スキルが加えるのは、整形（一文改行、太字と「」の使い分け、ダッシュと中黒の禁止）、視点と語り、「LLM っぽい表現の禁止」、冗長の排除である。
文書規模の規則（論証の順序、切り詰めテスト、図と参照）は `document-style` スキルが担う。
